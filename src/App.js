          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center w-full sm:w-auto justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-lg sm:text-2xl font-bold text-gray-800">TipScan</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        {showModal && pendingTip && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl sm:max-w-3xl max-h-[90vh] overflow-y-auto">
              <div className="p-6 sm:p-8">
                <h3 className="text-lg sm:text-2xl font-bold text-gray-800 mb-3">Distribución de Propina</h3>
                <p className="text-sm sm:text-base text-gray-600 mb-4">Propina de {formatCurrency(pendingTip.amount)} - Mesa {pendingTip.table}</p>
                
                <div className="bg-indigo-50 rounded-lg p-4 sm:p-6 mb-4">
                  <p className="text-sm font-semibold text-indigo-900 mb-3">¿Deseas usar la distribución estándar o personalizarla?</p>
                ...
                <div className="space-y-4 mb-4">
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-sm font-medium text-gray-700">Meseros</label>
                    ...
                    {useCustom ? (
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-3">
                        <div className="bg-indigo-600 h-3 rounded-full" style={{ width: `${defaultDistribution.meseros}%` }}></div>
                      </div>
                    )}
                    <p className="text-xs text-gray-500 mt-1">{useCustom ? customDistribution.meseros : defaultDistribution.meseros}%</p>
                  </div>
                  ...
                  <div>
                    ...
                    {useCustom ? (
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-3">
                        <div className="bg-purple-600 h-3 rounded-full" style={{ width: `${defaultDistribution.cocina}%` }}></div>
                      </div>
                    )}
                    <p className="text-xs text-gray-500 mt-1">{useCustom ? customDistribution.cocina : defaultDistribution.cocina}%</p>
                  </div>
                  
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-sm font-medium text-gray-700">Bar</label>
                    ...
                    {useCustom ? (
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-3">
                        <div className="bg-green-600 h-3 rounded-full" style={{ width: `${defaultDistribution.bar}%` }}></div>
                      </div>
                    )}
                    <p className="text-xs text-gray-500 mt-1">{useCustom ? customDistribution.bar : defaultDistribution.bar}%</p>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button
                    ...
                    className="flex-1 px-4 py-3 bg-gray-200 text-gray-700 rounded-lg font-semibold hover:bg-gray-300 transition-all text-sm"
                  >
                    Cancelar
                  </button>
                  <button
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6">
                ...
                <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg p-4 sm:p-6 text-white">
                  ...
                  <p className="text-xs sm:text-sm opacity-75 mt-1">{tips.length} propinas registradas</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {tips.filter(t => t.worker === selectedWorker).map(tip => (
                    <div key={tip.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-gray-50 rounded-lg p-3 sm:p-4 hover:bg-gray-100 transition-colors">
          ...
          <div className="max-w-2xl mx-auto px-2 sm:px-0">
            <div className="bg-white rounded-xl shadow-lg p-4 sm:p-8">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Monto de la Propina (COP)</label>
                  <input
                    type="number"
                    value={newTip.amount}
                    onChange={(e) => setNewTip({...newTip, amount: e.target.value})}
                    className="w-full px-3 py-2 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm"
                    placeholder="Ej: 5000"
                  />
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Tipo de Propina</label>
                  <div className="grid grid-cols-2 gap-3">
                    <button
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-3 sm:py-4 rounded-lg font-semibold hover:from-indigo-700 hover:to-purple-700 transition-all shadow-lg text-sm"
                >
                  Registrar Propina
                </button>
              </div>
        ...
        {view === 'public' && (
          <div className="max-w-4xl mx-auto px-2 sm:px-0">
            <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-xl shadow-2xl p-6 sm:p-12 text-white text-center">
              <Eye size={48} className="mx-auto mb-4 sm:mb-6 opacity-90" />
              <h2 className="text-xl sm:text-4xl font-bold mb-2">Gracias por tu Apoyo</h2>
              <p className="text-sm sm:text-xl mb-4 sm:mb-8 opacity-90">Hoy el equipo del restaurante ha recibido:</p>
              <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-6 sm:p-8 mb-6 sm:mb-8">
                <p className="text-3xl sm:text-6xl font-bold mb-1">{formatCurrency(totalTips)}</p>
                <p className="text-xs sm:text-lg opacity-90">en propinas</p>
              </div>
              ...
              <p className="mt-4 sm:mt-6 text-xs sm:text-sm opacity-75">Tu propina apoya directamente al equipo que te atendió.</p>
            </div>
          </div>
        )}
                ...
                <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 sm:p-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-3">
                        <div className="w-40 sm:w-48 bg-gray-200 rounded-full h-3">
